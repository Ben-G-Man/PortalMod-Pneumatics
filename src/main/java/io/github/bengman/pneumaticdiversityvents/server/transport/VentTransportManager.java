...
public final class VentTransportManager {
    ...
    private static final class WorldState {
        ...
        private void tickCaptured(ServerWorld world, VentSpatialRegistry registry) {
            ...
            while (iterator.hasNext()) {
                ...
                if (entity instanceof ServerPlayerEntity && Math.abs(netForce) >= MAX_SPEED_FORCE)
    ...
    public static double getTransportSpeed(int netForce) {
        ...
        if (magnitude >= MAX_SPEED_FORCE) return MAX_TRANSPORT_SPEED;
        double progress = (magnitude - 1.0D) / (MAX_SPEED_FORCE - 1.0D);
        return BASE_TRANSPORT_SPEED + (MAX_TRANSPORT_SPEED - BASE_TRANSPORT_SPEED) * progress;
    }
